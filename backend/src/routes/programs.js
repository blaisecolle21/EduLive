...
const {
  getActiveAnneeAcademiqueId,
} = require("../services/anneeAcademiqueService");
...
router.post(
  "/import",
  authMiddleware,
  checkPermission("program:import"),
  upload.single("pdf"),
  async (req, res) => {
    try {
      ...
      const promotionMatch = text.match(
        /Promotion\s*:\s*([0-9]+\s*e|[A-Z][a-z]*\s*[A-Z]|[\w\s]+?)(?:\s|$)/i,
      );
      ...
      const currentDiscipline = disciplineMatch
        ? disciplineMatch[1].trim()
        : "";
      ...
      const lignePattern =
        /(SEPT|OCT|NOV|DEC|JANV|FEV|MARS|AVRIL|MAI|JUIN)\s+(\d+)[- ]\s+(\d+(?:\/\d+)?\s+au\s+(?:\d+(?:\/\d+)?|[\d\/]+))\s+(SA\s*\d+(?:\s*\([^)]*\))?|CONGES|Période|PRE-RENTREE)/gi;
      ...
      for (let i = 0; i < matches.length; i++) {
        ...
        if (
          sa.match(
            /Période\s+d'intégration|CONGES|production\s+scolaire|Evaluation|Révision|Remédiation|Compte\s+rendu|Autres\s+activités/i,
          )
        ) {
        ...
        const tauxMatches = [
          ...fullContent.matchAll(/\b(\d+\.?\d*)\s+(\d+\.?\d*)\b/g),
        ];
        ...
        contentText = contentText.replace(
          /(SEPT|OCT|NOV|DEC|JANV|FEV|MARS|AVRIL|MAI|JUIN)\s+\d+[- ]/gi,
          "",
        );
        contentText = contentText.replace(
          /\b(SEPT|OCT|NOV|DEC|JANV|FEV|MARS|AVRIL|MAI|JUIN)\b/gi,
          "",
        );

        // 4. Supprimer les mentions de trimestre, congés, périodes
        contentText = contentText.replace(
          /\d+\s*(?:er|e|re|d)\s*(?:trimestre|semestre)/gi,
          "",
        );
        ...
        contentText = contentText.replace(
          /La\s+semaine\s+de\s+cours[^\n•]*/gi,
          "",
        );
        contentText = contentText.replace(
          /mais\s+est\s+interrompue[^\n•]*/gi,
          "",
        );
        ...
        for (let segment of segments) {
          ...
          // Si commence par "•"
            ...
            if (
              finalCleaned.length > 3 &&
              !finalCleaned.match(/^(fin|suite|début)$/i)
            ) {
              activites.push(finalCleaned);
            }
          }
      ...
      const insertedRecords = await models.ProgramTheorique.bulkCreate(
        entries,
        {
