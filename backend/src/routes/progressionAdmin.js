...
// ============================================================
// 1. VUE D'ENSEMBLE DES PROGRESSIONS POUR L'ADMINISTRATEUR
// ============================================================
router.get(
  "/admin/overview",
  authMiddleware,
  checkPermission("progression:view-all"), // MODIFICATION : RBAC appliqué
  async (req, res) => {
    try {
      ...
      const formattedClasses = await Promise.all(
        classes.map(async (classe) => {
          const disciplines = await Promise.all(
            (classe.Disciplines || []).map(async (discipline) => {
              ...
              if (enseignant) {
                try {
                  ...
                  console.log(
                    `✅ Discipline ${discipline.nom} - Progression: ${progression}% (${entriesCount} entrées, taux réalisé: ${tauxRealise})`,
                  );
                } catch (calcError) {
                  console.error(
                    `❌ Erreur calcul progression pour discipline ${discipline.id}:`,
                    calcError,
                  );
                }
              }
      ...
      console.log(
        `✅ Données formatées pour ${formattedClasses.length} classe(s)`,
      );
...
// ============================================================
// 2. DÉTAILS CHIRURGICAUX ET HISTORIQUE D'UNE DISCIPLINE
// ============================================================
router.get(
  "/admin/discipline/:disciplineId",
  authMiddleware,
  checkPermission("progression:view-detail"), // MODIFICATION : RBAC appliqué
  async (req, res) => {
    try {
      ...
      const entries = enseignant
        ? await models.CahierEntry.findAll({
            where: {
      ...
      console.log(
        `✅ Discipline ${discipline.nom} - ${entries.length} entrées, progression: ${progression}%`,
      );
...
// ============================================================
// 3. ROUTE DE DIAGNOSTIC / TEST DES COMPTEURS
// ============================================================
router.get(
  "/admin/test",
  authMiddleware,
  checkPermission("progression:view-all"), // MODIFICATION : Mutualisation de la permission view-all
  async (req, res) => {
    try {
