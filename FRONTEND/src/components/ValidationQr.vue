  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="max-w-lg w-full bg-white rounded-xl shadow p-6">
      ...
      <div v-if="entry" class="flex gap-2">
        ...
        <router-link
          to="/enseignant"
          class="flex-1 text-center bg-blue-100 text-blue-700 py-2.5 rounded-lg font-medium hover:bg-blue-200"
        >
          Modifier dans l'app
        </router-link>
      </div>
  ...
  methods: {
    async fetchEntry() {
      ...
      try {
        ...
        this.error =
          error.response?.data?.error || "Impossible de charger cette entrée.";
      } finally {
        this.loading = false;
      }
    },
